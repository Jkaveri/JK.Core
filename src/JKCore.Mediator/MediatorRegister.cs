...
namespace JKCore.Mediator
{
    ...
    public class MediatorRegister : ICommandHandlerResolver, IEventListenerResolver
    {
        ...
        public IAsyncCommandHandler<TCommand> ResolveAsyncHandler<TCommand>() where TCommand : IAsyncCommand
        {
            ...
            if (AnoHandlers.TryGetValue(cmdType, out handlerWrapper))
            {
                var handlerFn = handlerWrapper as Func<TCommand, Task<ICommandResult>>;
                if (handlerFn != null)
                {
                    return new AnonymousAsyncCommandHandler<TCommand>(handlerFn);
                }
            }
        ...
        public ICommandHandler<TCommand> ResolveHandler<TCommand>() where TCommand : ICommand
        {
            ...
            if (AnoHandlers.TryGetValue(cmdType, out handlerWrapper))
            {
                var handlerFn = handlerWrapper as Func<TCommand, ICommandResult>;
                if (handlerFn != null)
                {
                    return new AnonymousCommandHandler<TCommand>(handlerFn);
                }
            }
